...
// 📦 البيانات الافتراضية الأولية للتاجر المتنقل
const DEFAULT_CONTACTS: Contact[] = [
  {
    ...
    balance: -15000, // نطالبه/يطالبنا
    created_at: new Date(Date.now() - 86400000 * 5).toISOString(),
  },
  {
    id: 'c2',
    name: 'محمد العماري (زبون دائم)',
    phone: '213661987654',
    type: 'customer',
    notes: 'زبون دائم لديه دين متبقي',
    balance: 4500, // دين علينا له أو دين له علينا
...
const DEFAULT_PRODUCTS: Product[] = [
  ...
  },
  {
    ...
    retail_price: 1700,
    stock_quantity: 8,
    min_stock_alert: 10, // منخفض للتحذير
...
// 🛠️ مساعدات إدارة الحالة والمزامنة اليدوية المحلية
...
// 📲 إنشاء رابط واتساب ذكي جاهز للمشاركة
...
// 📄 مشاركة كشف حساب فوري بصيغة نصية احترافية
export function generateAccountStatementText(contactName: string, balance: number, transactions: Transaction[]): string {
  ...
  let statusText = balance > 0 
    ? `⚠️ الرصيد المتبقي المستحق عليك: ${balance.toLocaleString('ar-EG')} د.ج`
    : balance < 0 
    ? `✅ الرصيد المستحق لك لدينا: ${Math.abs(balance).toLocaleString('ar-EG')} د.ج`
    : `✅ الحساب مصفى بالكامل (0 د.ج)`;

  let msg = `🧾 *كشف حساب - التاجر الذكي*\n`;
